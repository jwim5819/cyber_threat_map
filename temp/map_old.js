...
// var webSock = new WebSocket(`ws://${window._env_.CLIENT_HOST}:${window._env_.CLIENT_PORT}/websocket`); // Internal
var webSock = new WebSocket("ws://192.168.10.231:8888/websocket"); // External
...
var map = L.mapbox.map("map").setView([0, 0], 2).addLayer(L.mapbox.styleLayer("mapbox://styles/mapbox/light-v10"));
...
var map = L.map('map', {
  center: [0, 0],
  zoom: 2,
  minZoom: 2,  // 줌 레벨 1로 설정
  maxZoom: 3   // 줌 레벨 3으로 설정
});
const tileUrl = '/static/mapbox_tiles/{z}/{x}/{y}.png';
L.tileLayer(tileUrl, {
  attribution: 'Mapbox Custom Style',
  tileSize: 512,
  zoomOffset: -1
}).addTo(map);
...
function handleTraffic(msg, srcPoint, hqPoint) {
  ...
  lineGraph
    ...
    .each("end", function () {
      d3.select(this).transition().duration(100).style("opacity", 0).remove();
    });
}
...
$(document).on("click", ".container-fluid .showInfo", function (e) {
  ...
  $("#informIP").html(
    "<a id='ip_only' href='" +
      iplink +
      "'></a><button id='exit'>X</button><h3>" +
      iplink +
      "</h3><br><ul><li><a target = '_blank' href='http://www.senderbase.org/lookup/?search_string=" +
      iplink +
      "'><b><u color=white>Senderbase</a></li><li><a target='_blank' href='https://ers.trendmicro.com/reputations/index'>Trend Micro</a></li><li><a target='_blank' href='http://www.anti-abuse.org/multi-rbl-check-results/?host=" +
      iplink +
      "'>Anti-abuse</a></li></ul><br><button id='blockIP' alt='" +
      iplink +
      "'>Block IP</button>   "
  );
});
